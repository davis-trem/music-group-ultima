...
class MIDIEventChunk:
	...
	## @param	_event			イベント
	func _init(_time:int = 0,_channel_number:int = 0,_event:MIDIEvent = null,_track_number:int = 0):
		self.time = _time
...
func _read_track( input:StreamPeerBuffer, track_number:int ) -> MIDITrack:
	...
	while 0 < stream.get_available_bytes( ):
		...
		events.append( MIDIEventChunk.new( time, event_type_byte & 0x0f, event, track_number ) )
